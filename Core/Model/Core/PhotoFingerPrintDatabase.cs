...
public struct PhotoFingerPrintDatabase : IFlatbufferObject
{
  ...
  internal PhotoFingerPrint? FingerPrints(int j) { int o = __p.__offset(4); return o != 0 ? (PhotoFingerPrint?)(new PhotoFingerPrint()).__assign(__p.__indirect(__p.__vector(o) + j * 4), __p.bb) : null; }
  ...
  internal static VectorOffset CreateFingerPrintsVector(FlatBufferBuilder builder, Offset<PhotoFingerPrint>[] data) { builder.StartVector(4, data.Length, 4); for (int i = data.Length - 1; i >= 0; i--) builder.AddOffset(data[i].Value); return builder.EndVector(); }
