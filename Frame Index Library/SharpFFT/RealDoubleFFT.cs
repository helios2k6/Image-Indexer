...
namespace SharpFFTPACK
{
    ...
    public class RealDoubleFFT : RealDoubleFFT_Mixed
    {
        ...
        /// <para>This is because a call of forward transform ft followed by a call of backward transform bt
        ...
        public void ft(double[] x, out Complex[] y)
        {
            ...
            y[0] = new Complex(x[0], 0);

            for (int i = 1; i < (Ndim + 1) / 2; i++)
            {
                y[i] = new Complex(x[2 * i - 1], x[2 * i]);
            }

            if (Ndim % 2 == 0)
            {
                y[Ndim / 2] = new Complex(x[Ndim - 1], 0);
            }
        }
        ...
        /// <summary>
        /// Backward real FFT transform. It is the unnormalized inverse transform of ft(double[], Complex1D).
        /// </summary>
